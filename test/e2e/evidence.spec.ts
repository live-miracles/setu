...
test('capture desktop evidence', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: `${output}/01-admin-home-desktop.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${output}/02-user-home-desktop.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${output}/06-program-conflict-desktop.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${output}/07-ticket-activity-desktop.png`, fullPage: false });

    await page.goto(`/?section=roster&${fast}`);
    await expect(page.getByText('August 2026')).toBeVisible();
    await page.screenshot({ path: `${output}/08-roster-month-desktop.png`, fullPage: false });

    await page.goto(`/?section=home-settings&${fast}`);
    await expect(page.getByRole('heading', { name: 'Home settings' })).toBeVisible();
    await page.screenshot({ path: `${output}/11-settings-desktop.png`, fullPage: false });
});
...
test('capture mobile evidence', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: `${output}/03-mobile-navigation.png`, fullPage: false });

    await page.goto(`/?section=inventory&${fast}`);
    await expect(page.getByRole('heading', { name: 'Inventory' })).toBeVisible();
    await page.screenshot({ path: `${output}/04-inventory-board-mobile.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${output}/09-roster-agenda-mobile.png`, fullPage: false });

    await page.goto(`/?${fast}&mockFailure=dashboard`);
    await expect(page.getByRole('button', { name: 'Try again' })).toBeFocused();
    await page.screenshot({ path: `${output}/12-error-retry-state-mobile.png`, fullPage: false });
});
