...
const DemoContext = createContext<
  { state: DemoState; actions: DemoActions } | undefined
>(undefined);
  ...
  const actions = useMemo<DemoActions>(
    () => ({
      ...
      async addTicketComment(id, commentMessage) {
        ...
        if (!isDemoMode) {
          await apiRequest(
            `/api/v1/tickets/${ticket.recordId ?? ticket.id}/comments`,
            {
              method: "POST",
              headers: { "Idempotency-Key": crypto.randomUUID() },
          ...
          const action =
            status === "closed"
              ...
              : ticket.status === "closed"
                ...
                : "assign";
          await apiRequest(
            `/api/v1/tickets/${ticket.recordId ?? ticket.id}/${action}`,
            {
              method: "POST",
              headers: { "Idempotency-Key": crypto.randomUUID() },
      ...
      async updateProfile(input) {
        ...
        setState((current) => ({
          ...
          profiles: current.profiles.map((profile) =>
            profile.id === current.currentUser.id
              ? { ...profile, ...input }
              : profile,
          ),
        }));
