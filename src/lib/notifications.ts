...
export async function dispatchPendingDeliveries(limit = 50) {
    ...
    for (const delivery of deliveries ?? []) {
        try {
            ...
            const notification = (
                Array.isArray(notificationValue) ? notificationValue[0] : notificationValue
            ) as {
                title: string;
                message: string;
                href: string;
...
async function sendEmail(notification: {
    ...
    const { error } = await resend.emails.send({
        from: process.env.RESEND_FROM_EMAIL ?? 'Livestream Operations <notifications@example.org>',
        to: profile.email,
