  ...
  const confirmAction = async () => {
    ...
    const note =
      actionDialog.nextStatus === "returned"
        ...
        : actionNote;
    try {
      await actions.transitionRequest(
        actionDialog.request.id,
        actionDialog.nextStatus,
        note,
      );
      message.success(
        `${actionDialog.request.id} moved to ${actionDialog.nextStatus}.`,
      );
    ...
    <>
      <div className="page-heading">
        <div>
          ...
          <p>
            Submit requests, approve handovers and record every return without
      ...
      <Card className="surface-card">
        <div className="toolbar">
          ...
          <span style={{ color: "#888b92", fontSize: 11 }}>
            {view === "Requests" ? requests.length : inventory.length} records ·
            live
          </span>
        </div>
      ...
      >
        {actionDialog?.nextStatus === "returned" && (
          <div style={{ marginBottom: 16 }}>
            <label
              style={{ display: "block", marginBottom: 7, fontWeight: 650 }}
            >
              Return condition
            </label>
            <Select
